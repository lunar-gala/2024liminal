...
export default function Model(props) { 
  ...
  useFrame(({ delta, pointer, clock }) => {
    groupRef.current.rotation.x = clock.getElapsedTime() /2.5
  })
  ...
  const config = useControls({
    ...
    bg: '#ffffff'
      ...
      const { rotation } = useControls({
        rotation: {
          ...
          step: .01, // step size for each value
        },
      })
    ...
    return (
          ...
          <boxGeometry 
            args={[2.15, .85, 0.01]}
            />
          <MeshTransmissionMaterial background={new THREE.Color(config.bg)} props={config} />
        </mesh>
          </>
